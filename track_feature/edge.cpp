...
void drawLine(Mat& imgOutput, Vec2f lines) {
	float theta = lines[1];
    float rho = lines[0];
    double a = cos(theta), b =sin(theta);
    double x0 = a*rho, y0 = b*rho;
...
void drawLine(Mat& imgOutput, Vec4f lines) {
	Point pt1(lines[2]-1000*lines[0], lines[3]-1000*lines[1]);
	Point pt2(lines[2]+1000*lines[0], lines[3]+1000*lines[1]);
...
vector<Point2f> TrackLine(Mat& first_frame, Mat& frame) {
    ...
    calcOpticalFlowPyrLK(first_frame, frame, bestPoints, new_points, status, err, window, max_level, criteria, flags, min_eigT );

    return new_points;

}

Vec4f fitLinesToNewPoint(vector<Point2f> points) {
	Vec4f newline;
	fitLine(points,newline,CV_DIST_L1,0,0.01,0.01);
	return newline;
  ...
  // 	processFirstFrame(first_frame);
    ...
    while(!finish){
	  ...
	  cout<<"Lost points!";
        ...
        NewLine = fitLinesToNewPoint(new_points);

        // drawLine(imgOutput, linesToTrack);
        
        drawLine(imgOutput, bestLine);
        drawPoint(imgOutput,bestPoints);
        drawLine(imgOutput, NewLine);
